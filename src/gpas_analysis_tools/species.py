...
def tidy_species(row):
    if 'tuberculosis' in row.MAPPED_SPECIES:
        ...
        lineage = row.LINEAGE
        sublineage = row.SUBLINEAGE
    return pandas.Series({'SPECIES': species, 'SUBSPECIES': subspecies, 'LINEAGE': lineage, 'SUBLINEAGE': sublineage})  
...
def parse_lineage(lineage_results):
    ...
    if lineage_results["Name"][:7] == "lineage":
        if lineage_results["Name"][7] in ["B","C",]:
...
def build_species_table(data_path = None, 
    tables_path = None,
    master_table = None, 
    max_samples = None, 
):
    ...
    for i in tqdm((data_path).rglob("*main_report*.json")):
        ...
        if (
            "Mycobacterial species identified" in data["Pipeline Outcome"]
        ):
            too_few_reads_genome += 1
            master_table.at[uid, "status"] = "cannot assemble"
            # continue
        elif (
            "Number of Mycobacterial reads is too low" in data["Pipeline Outcome"]
        ):
            too_few_reads_id += 1
            master_table.at[uid, "status"] = "cannot speciate"
            continue
        elif (
            "Sufficient reads mapped" in data["Pipeline Outcome"]
        ):
        ...
        master_table.at[uid, "HUMAN_READS"] = data['Organism Identification']['Human Reads']
        master_table.at[uid, "MYCOBACTERIAL_READS"] = data['Organism Identification']['Mycobacterium Reads']
        master_table.at[uid, "OTHER_BACTERIAL_READS"] = data['Organism Identification']['Non-Mycobacterium Bacteria Reads']
        master_table.at[uid, "UNCLASSIFIED_READS"] = data['Organism Identification']['Unclassified Reads']
        ...
        for sn in range(number_species):
            ...
            if has_new_block:
                ...
                if sn >= len(data["Assembled NTM Results"]["Lineage"]):
                    ...
                    lineage, sublineage = parse_lineage(data["Assembled NTM Results"]["Lineage"][sn])
            else:
                ...
                lineage, sublineage = parse_lineage(data["Mycobacterium Results"]["Lineage"][sn])
            row.append(lineage)
